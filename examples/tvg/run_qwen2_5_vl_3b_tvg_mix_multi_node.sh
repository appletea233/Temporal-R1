...
hope_ray_distributed_launch=$(python3 examples/hope_ray_distributed_launch.py)

is_master=$(python3 examples/hope_is_master.py)

echo ismaster: $is_master

echo $hope_ray_distributed_launch

if [[ "$is_master" == "1" ]];then
  $hope_ray_distributed_launch 
  ray job submit --address=http://127.0.0.1:8265 -- python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=qwen2_5_vl_3b_tvg_gpu_${GPUS}_v3_kl_${KL_LOSS_COEF}_n_${SAMPLE_N}_format_1_gbs_${GLOBAL_BS}_mix_large_multi_node \
  ...
  $hope_deepspeed_distributed_launch
  sleep infinity
fi
