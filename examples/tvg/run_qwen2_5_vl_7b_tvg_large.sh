...
MODEL_PATH=/mnt/dolphinfs/hdd_pool/docker/user/hadoop-mtcv/lihongyu/Qwen/Qwen2.5-VL-7B-Instruct  # replace it with your local file path  # replace it with your local file path
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=qwen2_5_vl_7b_tvg_node_${NODES}_gpu_${GPUS}_v3_kl_${KL_LOSS_COEF}_n_${SAMPLE_N}_large \
    trainer.n_gpus_per_node=$GPUS \
    trainer.val_generations_to_log=10 \
    trainer.save_freq=50 \
    trainer.val_before_train=false \
    trainer.logger=[\"console\",\"wandb\"] \
    data.min_pixels=2048 \
